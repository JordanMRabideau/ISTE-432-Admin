...
$(document).ready(function () {
      ...
      element.choices.forEach((choice, index) => {
        ...
        const option = `
          <div data-response=${response_id} class="choice">
            ...
            <label for="question-${question_id}-choice-${response_id}-title">Title: </label>
            <input value="${choice.title}" name="question-${question_id}-choice-${response_id}-title" data-type="title" id="question-${question_id}-choice-${response_id}-title" type="text" />&emsp;&emsp;
        ...
        $(`#question-${question_id}-choice-${response_id}-info`).val(
          choice.bio
        );
      });
  ...
  $("#questions").submit(function (e) {
    ...
    xhr("put", `http://localhost:3000/api/campaign/edit`, data).done(function (
      result
    ) {
